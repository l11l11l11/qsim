...
/// sim.create_process(Box::new(gen!({
///     yield_!( ItemState { stage: StageType::FirstPass,
...
pub type Process<T> = dyn Coroutine<Resume = SimContext<T>, Yield = T, Return = ()> + Unpin;
...
impl<T: 'static + SimState + Clone> Simulation<T> {
    ...
    pub fn create_process(&mut self, process: Box<Process<T>>) -> ProcessId {
    ...
    pub fn step(&mut self) {
        ...
        match self.future_events.pop() {
            Some(Reverse(event)) => {
                ...
                // process event
                match gstatepin {
                    genawaiter::GeneratorState::Yielded(y) => {
...
mod tests {
    ...
    fn it_works() {
        ...
        let p = s.create_process(Box::new(gen!({
            let mut a = 0.0;
            loop {
                a += 1.0;

                yield_!(Effect::TimeOut(a));
            }
        })));
    ...
    fn resource() {
        ...
        let mut s = Simulation::new();
        let r = s.create_resource(Box::new(SimpleResource::new(1)), "test");

        // simple process that lock the resource for 7 time units
        let p1 = s.create_process(Box::new(gen!({
            yield_!(Effect::Request(r));
            yield_!(Effect::TimeOut(7.0));
            yield_!(Effect::Release(r));
        })));
        // simple process that holds the resource for 3 time units
        let p2 = s.create_process(Box::new(gen!({
            yield_!(Effect::Request(r));
            yield_!(Effect::TimeOut(3.0));
            yield_!(Effect::Release(r));
        })));
